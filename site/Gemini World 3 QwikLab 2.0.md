...
To start, download this [Github repo](https://github.com/cszhu/build-with-gemini) that comes equipped with a selection of starter skills. 
...
From this new folder (`build-with-gemini`), run Antigravity CLI in the Cloud Shell.
...
Beyond skills, your workshop environment comes pre-wired with the tools you'll need to build agents on Google Cloud — no manual installation required. These load automatically when AGY starts up inside the `build-with-gemini` folder.
...
2. **AGY asks before it pushes.** Before creating the repo or pushing anything, AGY confirms the repo name and shows you what it's about to do. **Nothing is pushed to your account without your say-so** — so don't blindly approve; check the repo name and that it's your account first.
