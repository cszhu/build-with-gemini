...
Everything you've built so far lives on this remote desktop, and the desktop is temporary. When the lab ends, it goes away, and your code with it. So the last step is to push your project to **your own personal GitHub** so you keep it, can share it, and have a shot at the **Build with Gemini** project gallery.
...
2. **AGY asks before it pushes.** Before creating the repo or pushing anything, AGY confirms the repo name and shows you what it's about to do. **Nothing is pushed to your account without your say-so.** Don't blindly approve; check the repo name and that it's your account first.
   ...
   > **Name your repo `buildwithgemini-[YOUR-APP-NAME]`** (for example, `buildwithgemini-recipe-assistant`). AGY proposes this name by default, but if you rename it, keep this format.
...
When it's done, AGY prints your new repo's URL.
