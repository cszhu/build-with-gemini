...
> **Keep it real.** Your GIF must be the actual screen recording `record-demo` captured of your *running* app — never a drawn or AI-generated mock-up. If your agent is only partly working, film what *does* work (or skip the demo entirely) and say so in your README. A short, honest clip — or none — beats a polished fake.
...
> **Before you submit — a quick honesty check.** It's completely fine to submit a project that isn't fully working. The one thing we ask is honesty: your README should describe what your agent actually does *and* what it doesn't (yet), and any demo must be a real recording, never a mock-up. Submissions are reviewed, and standout projects are featured on merit.
